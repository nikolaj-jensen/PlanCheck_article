...
namespace plancheck_test
{
    ...
    class Plancheck_functions
    {
        ...
        public static int scan_dibh(ScriptContext context)
        {
            int evaluation = 1;
        ...
        public static int normalization(double norm_amount, string target, string volume, ScriptContext context)
        {
            ...
            switch (volume)
            {
                ...
                default:
                    ...
                    if (!(norm_amount.ToString() + ".00% covers " + volume + ".00% of Target Volume").ToLower().Equals(context.PlanSetup.PlanNormalizationMethod.ToLower(), StringComparison.OrdinalIgnoreCase))
                    {
                        evaluation = 3;
                    }
        ...
        public static int mlc_at_jaw(IEnumerable<Beam> beams, string mlc,double limit, out double distance, out double fraction)
        {
