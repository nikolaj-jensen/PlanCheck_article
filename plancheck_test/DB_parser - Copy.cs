...
namespace VMS.TPS
{
    ...
    {
        public static string[] doseplanner = { "kmat0038", "sjak0013", "lohl0001", "bkri0016", "tlar0028", "ktho0094", "dsch0003", "mehr0006","lstr0011" }; //BAM id for dosisplanlæggere, skal opdateres når der kommer nye
        ...
        public bool Read_diagnose(string diag_code, string hospital)
        {
            ...
            try
            {
                ...
                SqlCommand myCommand = new SqlCommand($"SELECT * FROM Plan_Diagnose_{hospital}_dec2020 WHERE diag_code='{diag_code.ToUpper()}';", SqlConnection); //Sådan ser en select ud
                ...
                while (myReader.Read())
                {
                    ...
                    if (!myReader.IsDBNull(0) || !myReader.IsDBNull(2) || !myReader.IsDBNull(3))
                    {
                        ...
                        {
                            DB_diag.DB_DIBH = myReader.GetInt32(8);
                        }
                        else
                        {
                            DB_diag.DB_DIBH = 0;
        ...
        public bool Read_priority(int diag_code, string hospital)
        {
            ...
            try
            {
                ...
                SqlCommand myCommand = new SqlCommand($"SELECT COUNT(idStruktur) FROM Plan_Prioritet_{hospital}_dec2020 WHERE idDiagnose_kode='{diag_code}';", SqlConnection); //Sådan ser en select ud
                myReader = myCommand.ExecuteReader();
                ...
                {
                    myCommand = new SqlCommand($"SELECT * FROM Plan_Prioritet_{hospital}_dec2020 WHERE idDiagnose_kode='{diag_code}';", SqlConnection); //Sådan ser en select ud
                    myReader = myCommand.ExecuteReader();
        ...
        public bool Read_structure(int struktur, string hospital)
        {
            ...
            try
            {
                ...
                SqlCommand myCommand = new SqlCommand($"SELECT * FROM Plan_Struktur_{hospital}_dec2020 WHERE idstruktur_tabel='{struktur}';", SqlConnection); //Sådan ser en select ud
        ...
        {
            bool db_result = false;
            if (Array.Exists(doseplanner,n=>n == user_ID.ToLower()))
            {
                db_result = true;
                return db_result;
            }
        ...
        public bool output_summary( string anon_ID, string course, string plan_name, string user_ID, string test_ID, int status, string hospital)
        {
            bool fail = true;
            ...
            using (SqlConnection SqlConnection = new SqlConnection(connStr))
            {
                try
                {
                    ...
                    try
                    {
                        string get_string = $"SELECT TOP 1 Fail, Total FROM Plan_summary_{hospital} WHERE Test ='{test_ID} ' ORDER BY id DESC";
                        ...
                        string add_string = $"INSERT into Plan_summary_{hospital} (Test, Time, Fail, Total) VALUES (  @test , @time , @fail , @total)"; //Sådan ser en select ud
                        using (SqlCommand myCommand = new SqlCommand(add_string, SqlConnection))
                        {
